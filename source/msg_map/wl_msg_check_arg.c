...
fsm_initialiser( check_arg,
    ...
    init_body (
        if (NULL == pchString || NULL == fnGetBytes || NULL == argc || NULL == argv  ) {
            abort_init();
        }
        this.pchStr = pchString;
        this.fnGetBytes = fnGetBytes;  
        this.hwCount = 0;		
        this.hwIndex = 0;
...
fsm_implementation(  check_arg )
{
    ...
    state(IS_START_OF_ARG) {
        if(this.bArgIsString == false){
            if(this.fnGetBytes((fsm(check_arg)*)&this,&(this.chByte),1)) { 
                if(this.chByte > 0){   
    ...
    state(RECEIVE_ARG) {
        if(this.bArgIsString == false){
            if(this.fnGetBytes((fsm(check_arg)*)&this,(uint8_t *)&(this.chArgLen),sizeof(this.chArgLen))) {       
                if(this.fnGetBytes((fsm(check_arg)*)&this,(uint8_t *)&(this.chArgs[this.hwIndex]),this.chArgLen)) { 
